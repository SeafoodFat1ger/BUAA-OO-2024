...
public class Strategy {
    ...
    public Advice getAdvice(int eleFloor, int elePersonNum, boolean eleDirection,
                            ...
                            int type, int stopFloor, ArrayList<Person> elePersonArr) {
        if (request.getResetRequest() != null) {
            return Advice.RESET;
        }
        if (request.getDcRequest() != null) {
            return Advice.DC;
        }
        ...
        if (eleFloor == stopFloor && request.isGoAway()) {
            return Advice.AWAY;
        }
        if (request.isGoAway()) {
            request.setGoAway(false);
        }
        //电梯是否有人->继续原方向
        if (elePersonNum != 0) {
            return Advice.MOVE;
    ...
    public boolean openForChange(int type, int stopFloor,
        ...
        //TimableOutput.println("OpenForChange?");
        if (type == 0 || eleFloor != stopFloor) {
            return false;
        }
