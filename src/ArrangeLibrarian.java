...
import static com.oocourse.library3.LibraryBookId.Type.B;
import static com.oocourse.library3.LibraryBookId.Type.C;
        ...
        for (Map.Entry<LibraryBookId, Integer> entry : borrowPool.getBooks().entrySet()) {
            if (entry.getValue() > 0) {
                ...
                if (entry.getKey().isTypeBU()) {
                    ...
                    library.getDriftLibrarian().updateDonScore(
    ...
    public boolean haveOrderB(String sid) {
        for (LibraryRequest request : needArrangeOrder) {
            if (request.getBookId().isTypeB()
                    && request.getStudentId().equals(sid)) {
                return true;
            }
        }
        return false;
