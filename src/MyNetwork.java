...
public class MyNetwork implements Network {
    ...
    public void sendMessage(int id) throws
        ...
        if (message instanceof EmojiMessage) {
            ...
            int heat = emojis.get(emojiId);
            emojis.put(emojiId, heat + 1);
        }
        ...
        if (message.getType() == 0) {
            ...
            Person person1 = message.getPerson1();
            person1.addSocialValue(message.getSocialValue());
            ((MyTag) message.getTag()).addSocialValue(message.getSocialValue());
            if (message instanceof RedEnvelopeMessage) {
                int size = message.getTag().getSize();
                if (size > 0) {
                    int i = ((RedEnvelopeMessage) message).getMoney() / size;
                    person1.addMoney(-i * size);
                    ((MyTag) message.getTag()).addMoney(i);
                }
            }
        }
