...
public class PathTest {//包含题目[3]
    ...
    public void testExtractLoopPath() {
        ...
        if (fl && oldPath.isValid()) {
            ...
            Object result = path.extractLoopPath();
            ...
            List<Integer> rnodes = ((Path) result).getNodes();

            //\result.nodes[0] == \result.nodes[\result.nodes.length - 1]
            assertEquals(rnodes.get(0), rnodes.get(rnodes.size() - 1));
            ...
            for (int i = 0; i < nodes.size() - 1; i++) {
                ...
                for (int j = 0; j < rnodes.size(); j++) {
                    if ((i + j) < nodes.size() && Objects.equals(rnodes.get(j), nodes.get(i + j))) {
                        continue;
                    }
                    flag = false;
                    break;
